...
export default function BookingFlow() {
  ...
  // ⭐ 页面显示语言（与 driver_lang 业务字段完全隔离，不写入 formData，不传给 API）
  ...
  useEffect(() => {
    ...
    // ⭐ 读取 UI 显示语言（与 step/order_id 完全独立，不写入 formData）
  ...
  // ⭐ 语言切换按钮（只切换显示文字，不改 URL / formData / localStorage）
  const langSwitcher = (
    <div
      style={{
        display: "flex",
  ...
  return (
    <div style={{ padding: "24px", maxWidth: "960px", margin: "0 auto" }}>
      ...
      {step === 6 && (
        <Step6Final initialData={formData} bookingUiLang={bookingUiLang} />
      )}
    </div>
  );
}

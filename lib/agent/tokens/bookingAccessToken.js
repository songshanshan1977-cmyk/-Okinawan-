...
//
// Secret comes ONLY from process.env.AGENT_BOOKING_TOKEN_SECRET — missing
// env var fails closed on both sign() and verify(), same posture as
...
function getSecret() {
  ...
  if (!secret || String(secret).trim().length === 0) return null;
  return secret;
}

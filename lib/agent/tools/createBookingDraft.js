...
// Postgres unique index on orders.agent_idempotency_key_hash (see the new
// migration) plus a single atomic `INSERT ... ON CONFLICT (...) DO NOTHING`
...
async function insertIdempotentDraft({ supabase, content, keyHash, requestHash }) {
  for (let attempt = 1; attempt <= MAX_ORDER_ID_ATTEMPTS; attempt++) {
    ...
    if (!error) {
      ...
      // data.length === 0: our row was not inserted because
      // agent_idempotency_key_hash already exists — that is the intended
      // "duplicate delivery of the same Idempotency-Key" outcome, not a
      // failure. The caller resolves it by reading the existing row.

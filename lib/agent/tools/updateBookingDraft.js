...
const ORDER_MERGE_COLUMNS = [
  ...
  "itinerary",
  "remark",
  // total_price/deposit_amount: NOT part of ALLOWED_CHANGE_FIELDS (they are
  // never accepted from `changes` — see the file header), but they ARE
  // part of lib/agent/bookingSummary.js's HASHED_FIELDS. computeSummaryHash()
  // is called directly on this pre-read row to compare against the
  // caller's expected_summary_hash — if either field were missing here,
  // that comparison would compute against `undefined` instead of the
  // order's real priced values, and a real Supabase read (which honors the
  // select() column list, unlike this project's queue-based test mock)
  // would make every genuine update spuriously fail with summary_stale.
  "total_price",
  "deposit_amount",
];

// pages/api/agent/update-booking-draft.js
//
// Agent tool endpoint: update_booking_draft (A2). Two auth layers, same
// pattern as pages/api/agent/get-booking-summary.js:
//   1. service-level: Authorization: Bearer <AGENT_SERVICE_KEY>
//   2. order-level:   X-Booking-Access-Token: <booking_access_token>
//
// order_id/expected_summary_hash/changes all come from the POST body; the
// booking_access_token from a dedicated header — never a query string.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await updateBookingDraftTool({
      supabase,
      order_id,
      expected_summary_hash: body.expected_summary_hash,

...
//
// Agent tool endpoint: confirm_booking_summary (A2). Same two-layer auth
// pattern as get-booking-summary.js / update-booking-draft.js:
//   1. service-level: Authorization: Bearer <AGENT_SERVICE_KEY>
//   2. order-level:   X-Booking-Access-Token: <booking_access_token>
//
// order_id/summary_hash come from the POST body; the booking_access_token
// from a dedicated header — never a query string.
...
import { confirmBookingSummaryTool } from "../../../lib/agent/tools/confirmBookingSummary";
import { logAgentToolCall } from "../../../lib/agent/audit";
import { AGENT_ERROR_CODES, statusForCode } from "../../../lib/agent/errorCodes";

const TOOL_NAME = "confirm_booking_summary";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await confirmBookingSummaryTool({ supabase, order_id, summary_hash: body.summary_hash });

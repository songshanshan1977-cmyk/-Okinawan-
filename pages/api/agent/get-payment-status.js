// pages/api/agent/get-payment-status.js
//
// Agent tool endpoint: get_payment_status (A3). Same two-layer auth pattern
// as every other pages/api/agent/*.js endpoint:
...
// dedicated header — never a query string. A pure read: never touches
// Stripe, never mutates the order.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await getPaymentStatusTool({ supabase, order_id });
